...
// Artnet settings
ArtnetWifi artnet;
const int startUniverse =
    0;  // CHANGE FOR YOUR SETUP most software this is 1, some software send
...
void DisplayManager_::drawJPG(int32_t x, int32_t y, const uint8_t jpeg_data[],
                              uint32_t data_size) {
    TJpgDec.drawJpg(x, y, jpeg_data, data_size);
}

void DisplayManager_::drawBMP(int16_t x, int16_t y, const uint16_t bitmap[],
                              int16_t w, int16_t h) {
...
void DisplayManager_::setAppTime(long duration) { ui->setTimePerApp(duration); }

void DisplayManager_::resetTextColor() { setTextColor(TEXTCOLOR_888); }
...
bool jpg_output(int16_t x, int16_t y, uint16_t w, uint16_t h,
                uint16_t *bitmap) {
...
void DisplayManager_::HSVtext(int16_t x, int16_t y, const char *text,
                              bool clear, byte textCase) {
    if (clear)
        matrix->clear();
...
void DisplayManager_::GradientText(int16_t x, int16_t y, const char *text,
                                   int color1, int color2, bool clear,
                                   byte textCase) {
    if (clear)
        matrix->clear();
    ...
    for (uint16_t i = 0; i < textLength; i++) {
        // Bestimme den Interpolationswert basierend auf der aktuellen Position i
        // im Text
        float t = (float)i / (textLength - 1);

        // Bestimme die Farbe für das aktuelle Zeichen basierend auf dem
        // Farbverlauf
...
void pushCustomApp(String name, int position) {
    if (customApps.count(name) == 0) {
        ...
        if (availableCallbackIndex == -1) {
            DEBUG_PRINTLN(
                F("Error adding custom app -> Maximum number of "
                  "custom apps reached"));
            return;
        }
...
bool parseFragmentsText(const JsonArray &fragmentArray,
                        std::vector<uint32_t> &colors,
                        std::vector<String> &fragments,
                        uint32_t standardColor) {
    colors.clear();
    fragments.clear();
...
bool DisplayManager_::generateCustomPage(const String &name, JsonObject doc,
                                         bool preventSave) {
    CustomApp customApp;
    ...
    customApp.progress =
        doc.containsKey("progress") ? doc["progress"].as<int>() : -1;
    ...
    if (doc.containsKey("save") && preventSave == false) {
        ...
        if (saveApp) {
            ...
            // Open the file for writing (this will overwrite the file if it
            // already exists)
    ...
    if (doc.containsKey("effect")) {
        ...
        if (doc.containsKey("effectSettings")) {
            updateEffectSettings(customApp.effect,
                                 doc["effectSettings"].as<String>());
        }
    }
    customApp.duration =
        doc.containsKey("duration") ? doc["duration"].as<long>() * 1000 : 0;
    ...
    if (doc.containsKey("lifetime")) {
        ...
        customApp.lifetimeMode =
            doc.containsKey("lifetimeMode") ? doc["lifetimeMode"] : 0;
    } else {
    ...
    customApp.scrollSpeed =
        doc.containsKey("scrollSpeed") ? doc["scrollSpeed"].as<int>() : -1;
    customApp.topText =
        doc.containsKey("topText") ? doc["topText"].as<bool>() : false;
    customApp.fade = doc.containsKey("fadeText") ? doc["fadeText"].as<int>() : 0;
    customApp.blink =
        doc.containsKey("blinkText") ? doc["blinkText"].as<int>() : 0;
    customApp.center =
        doc.containsKey("center") ? doc["center"].as<bool>() : true;
    ...
    if (doc.containsKey("icon")) {
        ...
        if (newIconName.endsWith("=")) {
            customApp.jpegDataSize = decode_base64(
                (const unsigned char *)newIconName.c_str(), customApp.jpegDataBuffer);
    ...
    if (doc.containsKey("text") && doc["text"].is<JsonArray>()) {
        ...
        parseFragmentsText(textArray, customApp.colors, customApp.fragments,
                           customApp.color);
        // Der Code zur Zuweisung von customApp.text könnte hier angepasst
        // werden, je nachdem, wie Sie die Textinformationen verwenden möchten.
...
bool DisplayManager_::generateNotification(uint8_t source, const char *json) {
    ...
    if (doc.containsKey("effect")) {
        ...
        if (doc.containsKey("effectSettings")) {
            updateEffectSettings(newNotification.effect,
                                 doc["effectSettings"].as<String>());
        }
    }

    newNotification.loopSound =
        doc.containsKey("loopSound") ? doc["loopSound"].as<bool>() : false;
    newNotification.center =
        doc.containsKey("center") ? doc["center"].as<bool>() : true;
    newNotification.sound =
        doc.containsKey("sound") ? doc["sound"].as<String>() : "";
    newNotification.rtttl =
        doc.containsKey("rtttl") ? doc["rtttl"].as<String>() : "";
    newNotification.duration = doc.containsKey("duration")
                                   ? doc["duration"].as<long>() * 1000
                                   : TIME_PER_APP;
    newNotification.rainbow =
        doc.containsKey("rainbow") ? doc["rainbow"].as<bool>() : false;
    newNotification.hold =
        doc.containsKey("hold") ? doc["hold"].as<bool>() : false;
    newNotification.scrollSpeed =
        doc.containsKey("scrollSpeed") ? doc["scrollSpeed"].as<int>() : -1;
    newNotification.wakeup =
        doc.containsKey("wakeup") ? doc["wakeup"].as<bool>() : false;
    newNotification.pushIcon = doc.containsKey("pushIcon") ? doc["pushIcon"] : 0;
    newNotification.iconOffset =
        doc.containsKey("iconOffset") ? doc["iconOffset"] : 0;
    newNotification.textCase = doc.containsKey("textCase") ? doc["textCase"] : 0;
    newNotification.textOffset =
        doc.containsKey("textOffset") ? doc["textOffset"] : 0;
    newNotification.topText =
        doc.containsKey("topText") ? doc["topText"].as<bool>() : false;
    newNotification.noScrolling =
        doc.containsKey("noScroll") ? doc["noScroll"] : false;
    newNotification.repeat =
        doc.containsKey("repeat") ? doc["repeat"].as<int>() : -1;
    newNotification.fade =
        doc.containsKey("fadeText") ? doc["fadeText"].as<int>() : 0;
    ...
    if (doc.containsKey("gradient")) {
        ...
        if (arr.size() == 2) {
            ...
            newNotification.gradient[0] =
                getColorFromJsonVariant(color1, TEXTCOLOR_888);
            newNotification.gradient[1] =
                getColorFromJsonVariant(color2, TEXTCOLOR_888);
        }
    }
    ...
    if (doc.containsKey("text") && doc["text"].is<JsonArray>()) {
        JsonArray textArray = doc["text"].as<JsonArray>();
        parseFragmentsText(textArray, newNotification.colors,
                           newNotification.fragments, newNotification.color);
    ...
    if (doc.containsKey("icon")) {
        ...
        if (iconValue.endsWith("=")) {
            newNotification.jpegDataSize =
                decode_base64((const unsigned char *)iconValue.c_str(),
                              newNotification.jpegDataBuffer);
...
void DisplayManager_::loadCustomApps() {
    ...
    while (file) {
        ...
        String name = fileName.substring(
            ...
            fileName.lastIndexOf('.'));  // remove path and .json extension
...
void DisplayManager_::loadNativeApps() {
    ...
    auto updateApp = [&](const String &name, AppCallback callback, bool show,
                         size_t position) {
        auto it = std::find_if(Apps.begin(), Apps.end(),
                               [&](const std::pair<String, AppCallback> &app) {
...
#endif
    updateApp("Counter", CounterApp, SHOW_CNT, 3);
...
void checkLifetime(uint8_t pos) {
    ...
    if (appIt != customApps.end()) {
        ...
        if (app.lifetime > 0 &&
            (millis() - app.lastUpdate) / 1000 >= app.lifetime) {
...
void onDmxFrame(uint16_t universe, uint16_t length, uint8_t sequence,
    ...
    for (int i = 0; i < length / 3; i++) {
        ...
        if (led < 256)
            matrix->drawPixel(
                led % matrix->width(), led / matrix->width(),
                matrix->Color(data[i * 3], data[i * 3 + 1], data[i * 3 + 2]));
    }
...
void DisplayManager_::drawProgressBar(int16_t x, int16_t y, int progress,
...
void DisplayManager_::drawBarChart(int16_t x, int16_t y, const int newData[],
                                   byte dataSize, bool withIcon,
                                   uint32_t color) {
...
void DisplayManager_::drawLineChart(int16_t x, int16_t y, const int newData[],
                                    byte dataSize, bool withIcon,
                                    uint32_t color) {
    int availableWidth = withIcon ? (32 - 9) : 32;
    int startX = withIcon ? 9 : 0;
    float xStep =
        static_cast<float>(availableWidth) / static_cast<float>(dataSize - 1);
    int lastX = x + startX;
...
std::pair<String, AppCallback> getNativeAppByName(const String &appName) {
    ...
    } else if (appName == "Counter") {
        return std::make_pair("Counter", CounterApp);
    }
#ifdef ULANZI
...
void DisplayManager_::updateAppVector(const char *json) {
    ...
    for (JsonObject appObj : appArray) {
        ...
        int position =
            appObj.containsKey("pos") ? appObj["pos"].as<int>() : Apps.size();
...
String DisplayManager_::getStats() {
    ...
    if (SENSOR_READING) {
        double formattedTemp =
            roundToDecimalPlaces(CURRENT_TEMP, TEMP_DECIMAL_PLACES);
...
void DisplayManager_::setMatrixLayout(int layout) {
    ...
    switch (layout) {
        ...
        case 2:
            matrix =
                new FastLED_NeoMatrix(leds, 32, 8,
                                      NEO_MATRIX_TOP + NEO_MATRIX_LEFT +
    ...
    ui = new MatrixDisplayUi(
        matrix);  // Create a new ui object with the new matrix
}
...
void DisplayManager_::powerStateParse(const char *json) {
    ...
    DeserializationError error = deserializeJson(doc, json);
    if (error) {
        setPower((strcmp(json, "true") == 0 || strcmp(json, "1") == 0) ? true
                                                                       : false);
        return;
    }
...
bool DisplayManager_::indicatorParser(uint8_t indicator, const char *json) {
    if (strcmp(json, "") == 0 || strcmp(json, "{}") == 0) {
        switch (indicator) {
            case 1:
                ...
                MQTTManager.setIndicatorState(1, ui->indicator1State,
                                              ui->indicator1Color);
                break;
            case 2:
                ...
                MQTTManager.setIndicatorState(2, ui->indicator2State,
                                              ui->indicator2Color);
                break;
            case 3:
                ...
                MQTTManager.setIndicatorState(3, ui->indicator3State,
                                              ui->indicator3Color);
                break;
            default:
                break;
    ...
    doc.clear();
    MQTTManager.setIndicatorState(indicator, ui->indicator1State,
                                  ui->indicator1Color);
    return true;
}
...
float logMap(float x, float in_min, float in_max, float out_min, float out_max,
    ...
    if (x <= (in_max + in_min) / 2.0) {
        ...
        float upper_scale =
            (out_max - mid_point_out) / log(in_max - (in_max + in_min) / 2.0 + 1);
...
void DisplayManager_::setNewSettings(const char *json) {
    ...
    TIME_PER_TRANSITION =
        doc.containsKey("TSPEED") ? doc["TSPEED"] : TIME_PER_TRANSITION;
    ...
    START_ON_MONDAY =
        doc.containsKey("SOM") ? doc["SOM"].as<bool>() : START_ON_MONDAY;
    MATRIX_OFF = doc.containsKey("MATP") ? !doc["MATP"].as<bool>() : MATRIX_OFF;
    TIME_FORMAT =
        doc.containsKey("TFORMAT") ? doc["TFORMAT"].as<String>() : TIME_FORMAT;
    GAMMA = doc.containsKey("GAMMA") ? doc["GAMMA"].as<float>() : GAMMA;
    DATE_FORMAT =
        doc.containsKey("DFORMAT") ? doc["DFORMAT"].as<String>() : DATE_FORMAT;
    AUTO_BRIGHTNESS =
        doc.containsKey("ABRI") ? doc["ABRI"].as<bool>() : AUTO_BRIGHTNESS;
    AUTO_TRANSITION =
        doc.containsKey("ATRANS") ? doc["ATRANS"].as<bool>() : AUTO_TRANSITION;
    UPPERCASE_LETTERS = doc.containsKey("UPPERCASE") ? doc["UPPERCASE"].as<bool>()
                                                     : UPPERCASE_LETTERS;
    SHOW_WEEKDAY = doc.containsKey("WD") ? doc["WD"].as<bool>() : SHOW_WEEKDAY;
    BLOCK_NAVIGATION =
        doc.containsKey("BLOCKN") ? doc["BLOCKN"].as<bool>() : BLOCK_NAVIGATION;
...
String DisplayManager_::ledsAsJson() {
    ...
    for (int y = 0; y < MATRIX_HEIGHT; y++) {
        for (int x = 0; x < MATRIX_WIDTH; x++) {
            ...
            int color = (ledsCopy[index].r << 16) | (ledsCopy[index].g << 8) |
                        ledsCopy[index].b;
...
void DisplayManager_::processDrawInstructions(int16_t xOffset, int16_t yOffset,
...
CRGB *DisplayManager_::getLeds() { return leds; }
...
void DisplayManager_::drawRect(int16_t x, int16_t y, int16_t w, int16_t h,
                               uint32_t color) {
...
void DisplayManager_::drawFastVLine(int16_t x, int16_t y, int16_t h,
                                    uint32_t color) {
    drawLine(x, y, x, y + h - 1, color);
}

void DisplayManager_::drawFilledRect(int16_t x, int16_t y, int16_t w, int16_t h,
                                     uint32_t color) {
    for (int16_t i = x; i < x + w; i++) {
...
void DisplayManager_::drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1,
                               uint32_t color) {
...
void DisplayManager_::drawRGBBitmap(int16_t x, int16_t y, uint32_t *bitmap,
                                    int16_t w, int16_t h) {
...
void DisplayManager_::drawCircle(int16_t x0, int16_t y0, int16_t r,
                                 uint32_t color) {
    int16_t x = r;
    int16_t y = 0;
    int16_t p = 1 - r;
...
void DisplayManager_::fillCircle(int16_t x0, int16_t y0, int16_t r,
                                 uint32_t color) {
    matrix->drawPixel(x0, y0, color);
...
void DisplayManager_::matrixPrint(String str) { matrixPrint(str.c_str()); }
...
void DisplayManager_::setTextColor(uint32_t color) { textColor = color; }